...
import { getWatchedListFire, getQueueListFire } from './firebase/dataBase';
...
async function checkStatus(id) {
  ...
  let chooseFilmWatched = -1;
  let chooseFilmQueue = -1;
  ...
  const watchedCheck = await getWatchedListFire();
  const queueCheck = await getQueueListFire();

  if (watchedCheck.includes(Number(id))) {
    chooseFilmWatched = 1;
  } else {
    chooseFilmWatched = -1;
  }
  if (queueCheck.includes(Number(id))) {
    chooseFilmQueue = 1;
  } else {
    chooseFilmQueue = -1;
  }

  const watched = chooseFilmWatched < 0 ? false : true;
  console.log('watched: ', watched);
  const queque = chooseFilmQueue < 0 ? false : true;
  console.log('queque: ', queque);
  return { watched, queque };
}

export async function marckupBtn(id) {
  ...
  const marckupQuequeText = status.queque
    ? 'remove from queue'
    : 'add to queue';
